...
#define PITCH_GRAVITY_FF_K -9000.0f
...
#define PITCH_HOLD_DEADBAND_DEG 0.3f
...
static void ProcessGimbalControl(void)
{
    ...
    if (pitch_manual_active && pitch_motor != NULL) {
...
static void UpdateGimbalControl(void)
{
    ...
    if (!pitch_manual_active) {
        ...
        if (fabsf(error) > PITCH_HOLD_DEADBAND_DEG) {
            pitch_hold_speed = float_constrain(error * PITCH_HOLD_KP, -PITCH_HOLD_MAX_SPEED,
