...
enum {
    BMI088_REGISTER_VERIFY_ATTEMPTS = 3,
    BMI088_CALIBRATION_SAMPLES = 6000,
};
...
static bool Calibrate_MPU_Offset(IMU_Data_t *bmi088,
                                 uint32_t init_start_ms,
                                 uint32_t timeout_ms);
static void Load_Offline_Calibration(IMU_Data_t *bmi088);

static bool Init_Timeout_Expired(uint32_t init_start_ms, uint32_t timeout_ms)
{
    return RmTime_ElapsedMs(RmTime_NowMs(), init_start_ms) >= timeout_ms;
}

uint8_t BMI088Init(SPI_HandleTypeDef *bmi088_SPI, uint8_t calibrate)
{
    return BMI088InitWithTimeout(bmi088_SPI, calibrate,
                                 BMI088_DEFAULT_INIT_TIMEOUT_MS);
}

uint8_t BMI088InitWithTimeout(SPI_HandleTypeDef *bmi088_SPI,
                              uint8_t calibrate,
                              uint32_t timeout_ms)
{
    uint32_t init_start_ms;
    ...
    if ((bmi088_SPI == NULL) || (timeout_ms == 0U)) {
        return BMI088_INVALID_ARGUMENT_ERROR;
    }

    BMI088_SPI = bmi088_SPI;
    init_start_ms = RmTime_NowMs();

    init_error = bmi088_accel_init();
    if (init_error != BMI088_NO_ERROR) {
        return init_error;
    }
    if (Init_Timeout_Expired(init_start_ms, timeout_ms)) {
        return BMI088_INIT_TIMEOUT_ERROR;
    }
    ...
    if (calibrate != 0U) {
        if (!Calibrate_MPU_Offset(&BMI088, init_start_ms, timeout_ms)) {
            return BMI088_CALIBRATION_ERROR;
        }
    } else {
        Load_Offline_Calibration(&BMI088);
    }
                                 ...
                                 uint32_t timeout_ms)
{
    ...
    float gyroDiff[3] = {0.0f, 0.0f, 0.0f};
    ...
    for (;;) {
        ...
        DWT_Delay(0.005);
        bmi088->gNorm = 0.0f;
        bmi088->GyroOffset[0] = 0.0f;
        bmi088->GyroOffset[1] = 0.0f;
        bmi088->GyroOffset[2] = 0.0f;

        for (uint16_t i = 0; i < BMI088_CALIBRATION_SAMPLES; ++i) {
            if (Init_Timeout_Expired(init_start_ms, timeout_ms)) {
                stable = false;
                break;
            }
                ...
                gyroDiff[2] > 0.15f)
            {
                if (!interruption_logged) {
                    LOGWARNING("[bmi088] calibration interrupted; waiting for stillness");
        ...
        if (!stable || (sample_count != BMI088_CALIBRATION_SAMPLES)) {
            continue;
        }

        bmi088->gNorm /= (float)sample_count;
        for (uint8_t i = 0; i < 3; ++i) {
            bmi088->GyroOffset[i] /= (float)sample_count;
        }
    ...
    Load_Offline_Calibration(bmi088);
    LOGERROR("[BMI088] calibration timed out; offline parameters loaded");
    return false;
}
...
uint8_t bmi088_accel_init(void)
{
    ...
    for (uint8_t reg_num = 0; reg_num < BMI088_WRITE_ACCEL_REG_NUM;
         reg_num++) {
        uint8_t attempt;

        for (attempt = 0; attempt < BMI088_REGISTER_VERIFY_ATTEMPTS;
             attempt++) {
            BMI088_accel_write_single_reg(BMI088_Accel_Init_Table[reg_num][0],
                                          BMI088_Accel_Init_Table[reg_num][1]);
            DWT_Delay(0.001);
            BMI088_accel_read_single_reg(BMI088_Accel_Init_Table[reg_num][0],
                                         res);
            DWT_Delay(0.001);
            if (res == BMI088_Accel_Init_Table[reg_num][1]) {
                break;
            }
        }
        if (attempt == BMI088_REGISTER_VERIFY_ATTEMPTS) {
            return BMI088_Accel_Init_Table[reg_num][2];
        }
    }
    return BMI088_NO_ERROR;
}
...
uint8_t bmi088_gyro_init(void)
{
    ...
    for (uint8_t reg_num = 0; reg_num < BMI088_WRITE_GYRO_REG_NUM;
         reg_num++) {
        uint8_t attempt;

        for (attempt = 0; attempt < BMI088_REGISTER_VERIFY_ATTEMPTS;
             attempt++) {
            BMI088_gyro_write_single_reg(BMI088_Gyro_Init_Table[reg_num][0],
                                         BMI088_Gyro_Init_Table[reg_num][1]);
            DWT_Delay(0.001);
            BMI088_gyro_read_single_reg(BMI088_Gyro_Init_Table[reg_num][0],
                                        res);
            DWT_Delay(0.001);
            if (res == BMI088_Gyro_Init_Table[reg_num][1]) {
                break;
            }
        }
        if (attempt == BMI088_REGISTER_VERIFY_ATTEMPTS) {
            return BMI088_Gyro_Init_Table[reg_num][2];
        }
    }
