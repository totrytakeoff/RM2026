...
- 遥控：ET08 SBUS（参照 `tests/firmware/basic/et08_sbus_demo` 的通信逻辑）
...
2) 单独烧录 `tests/firmware/basic/omni_chassis_demo` 验证 CAN1
3) 在 `tests/firmware/infantry/infantry_omni_demo` 中打开 `CHASSIS_FORCE_TEST` 强制输出
4) 若仍无响应，倾向硬件/总线层问题
