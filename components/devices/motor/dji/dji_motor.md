...
4. ==GM6020的输入设定为**力矩**，待测量（-30000~30000）==
   ...
   `DJIMotorControl()`会在所有 PID、前馈和方向变换之后按电机型号执行最终限幅，
   因此应用层误设的 `MaxOut` 不会越过对应电调的合法 CAN 命令范围。
...
- `DJIMotorGetTuningSnapshot()`统一返回电机反馈与最近一次完整控制计算的运行态，
  只描述 DJI 电机本身。底盘、云台、发射等应用状态应通过各自模块的诊断接口提供，
  不得加入通用电机快照。
