# 步兵底盘云台 ET08 备用接线融合测试

文档更新日期：2026-07-19

该 target 保留一套备用底盘电机接线映射：`FL=2`、`FR=1`、`BL=3`、`BR=4`。
除接线映射外，控制目的与主版本 `infantry_chassis_gimbal_et08_demo` 一致；仅在实车线束
符合上述 CAN ID 分配时使用，避免再以 `demo1` 之类临时编号区分硬件版本。
...
```bash
cmake -S . -B build
cmake --build build --target test_infantry_chassis_gimbal_et08_alt_wiring_demo -j8
```

## 烧录

```bash
cmake --build build --target flash-test_infantry_chassis_gimbal_et08_alt_wiring_demo
```
