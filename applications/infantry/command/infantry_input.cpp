...
#elif INFANTRY_REMOTE_BACKEND == INFANTRY_REMOTE_BACKEND_DT7
#include "dt7_backend.hpp"
using ActiveRemoteBackend = rm::remote::Dt7Backend;
#elif INFANTRY_REMOTE_BACKEND == INFANTRY_REMOTE_BACKEND_VT
#include "vt_backend.hpp"
using ActiveRemoteBackend = rm::remote::VtBackend;
#else
#error "Unsupported INFANTRY_REMOTE_BACKEND"
#endif
...
{
#if INFANTRY_REMOTE_BACKEND == INFANTRY_REMOTE_BACKEND_DT7
...
static void MapSelectedRemote(const RemoteControlState &state,
                              Input_Data_t &data)
{
    const rm::infantry::Et08InputProfileConfig config = {
        ET08_STICK_DEADZONE_RAW,
        ET08_STICK_FULL_SCALE_RAW,
    };

    (void)rm::infantry::MapEt08Input(state, config, data);
}
#elif INFANTRY_REMOTE_BACKEND == INFANTRY_REMOTE_BACKEND_VT
static void MapSelectedRemote(const RemoteControlState &state,
                              Input_Data_t &data)
{
    ...
    data.control_mode = INFANTRY_CONTROL_FOLLOW;
    data.fire_mode = INFANTRY_FIRE_DISABLED;
...
extern "C" bool Input_Init(void)
{
    ...
    MDBG_IN("selected remote backend=%u init=%u",
            static_cast<unsigned>(INFANTRY_REMOTE_BACKEND),
            initialized ? 1U : 0U);
    return initialized;
}
