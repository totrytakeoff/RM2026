...
/*
 * 云台与底盘对齐时Yaw编码器零位.
 * 该值与 Src/application/robot_def.h 中 infantry 默认参数保持一致.
 */
 ...
 * P项先保证最小demo可跑,后续可再补底盘陀螺仪或更强阻尼.
 */
#define CHASSIS_FOLLOW_WZ_KP 0.10f
#define CHASSIS_FOLLOW_WZ_KD 0.0025f
#define CHASSIS_FOLLOW_WZ_MAX 8.0f
...
static float GetYawOffsetDeg(void);
...
static bool BuildChassisCommandFromEt08(ChassisCmd_t *cmd)
{
    ...
    yaw_offset_deg = GetYawOffsetDeg();
    cmd->yaw_offset_deg = yaw_offset_deg;
    cos_theta = cosf(yaw_offset_deg * (float)M_PI / 180.0f);
    sin_theta = sinf(yaw_offset_deg * (float)M_PI / 180.0f);
    ...
    if (chassis_mode == CHASSIS_MODE_FOLLOW) {
        ...
        cmd->wz_cmd =
            -(CHASSIS_FOLLOW_WZ_KP * yaw_offset_deg + CHASSIS_FOLLOW_WZ_KD * yaw_offset_speed_deg) *
            ((float)M_PI / 180.0f);
...
static void SendTelemetry(void)
{
    ...
    TelemetryPrintf("[chs_gmb_et08] yaw imu(ref_x10=%ld fdb_x10=%ld gz_x10=%ld) enc(total_x10=%ld off_x10=%ld)\r\n",
...
static void SendVofaFrame(void)
{
    ...
    ch[5] = (gimbal_imu != NULL) ? gimbal_imu->Yaw : 0.0f;
